...
FlatMeshTriangle::FlatMeshTriangle(std::shared_ptr<Mesh> mesh_ptr, const int i0, const int i1, const int i2)
...
std::shared_ptr<FlatMeshTriangle> FlatMeshTriangle::Clone() const
{
	return std::make_shared<FlatMeshTriangle>(*this);
}

bool FlatMeshTriangle::hit(const Ray& ray, double& tMin, ShadeRec& sr)
{
