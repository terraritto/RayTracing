#include "Constants.h"
int SolveQuadric(std::array<double, 3> c, std::array<double, 2>& s)
{
	double p, q, D;

	// normal form: x^2 + px + q = 0
	p = c[1] / (2 * c[2]);
	q = c[0] / c[2];

	D = p * p - q;

	if (IsZero(D))
	{
		s[0] = -p;
		return 1;
	}
	else if (D < 0)
	{
		return 0;
	}
	else
	{
		double sqrt_D = std::sqrt(D);

		s[0] = sqrt_D - p;
		s[1] = -sqrt_D - p;
		return 2;
	}
}

int SolveCubic(std::array<double, 4> c, std::array<double, 3>& s)
{
	int num;
	double sub;
	double A, B, C;
	double sq_A, p, q;
	double cb_p, D;

	// normal form: x^3 + Ax^2 + Bx + C = 0
	A = c[2] / c[3];
	B = c[1] / c[3];
	C = c[0] / c[3];

	// substitute x = y - A/3 to eliminate quadric term:
	// x^3 + px + q = 0

	sq_A = A * A;
	p = 1.0 / 3 * (-1.0 / 3 * sq_A + B);
	q = 1.0 / 2 * (2.0 / 27 * A * sq_A - 1.0 / 3 * A * B + C);

	// use Cardano's formula
	cb_p = p * p * p;
	D = q * q + cb_p;
	if (IsZero(D)) {
		if (IsZero(q)) // one triple solution
		{
			s[0] = 0;
			num = 1;
		}
		else // one single and one double solution
		{
			double u = cbrt(-q);
			s[0] = 2 * u;
			s[1] = -u;
			num = 2;
		}
	}
	else if (D < 0) // Casus irredcibilis: three real solutions
	{
		double phi = 1.0 / 3 * std::acos(-q / std::sqrt(-cb_p));
		double t = 2 * std::sqrt(-p);

		s[0] = t * std::cos(phi);
		s[1] = -t * cos(phi + PI / 3);
		s[2] = -t * cos(phi - PI / 3);
		num = 3;
	}
	else
	{
		double sqrt_D = std::sqrt(D);
		double u = cbrt(sqrt_D - q);
		double v = -cbrt(sqrt_D + q);

		s[0] = u + v;
		num = 1;
	}

	// resubstitute
	sub = 1.0 / 3 * A;

	for (int i = 0; i < num; ++i)
	{
		s[i] -= sub;
	}

	return num;
}

int SolveQuartic(std::array<double, 5> c, std::array<double, 4>& s)
{
	std::array<double, 4> coeffs;
	std::array<double, 3> temp1, temp2;
	std::array<double, 2> temp3;
	double z, u, v, sub;
	double A, B, C, D;
	double sq_A, p, q, r;
	int num;

	// normal form: x^4 + Ax^3 + Bx^2 + Cx + D = 0
	A = c[3] / c[4];
	B = c[2] / c[4];
	C = c[1] / c[4];
	D = c[0] / c[4];
	// substitute x = y - A/4 to eliminate
	// cubic term: x^4 + px^2 + qx + r = 0
	sq_A = A * A;
	p = -3.0 / 8 * sq_A + B;
	q = 1.0 / 8 * sq_A * A - 1.0 / 2 * A * B + C;
	r = -3.0 / 256 * sq_A * sq_A + 1.0 / 16 * sq_A * B - 1.0 / 4 * A * C + D;

	if (IsZero(r))
	{
		// no absolute term: y(y^3 + py + q) = 0

		coeffs[0] = q;
		coeffs[1] = p;
		coeffs[2] = 0;
		coeffs[3] = 1;

		num = SolveCubic(coeffs, temp1);
		for (int i = 0; i < 3; i++)
		{
			s[i] = temp1[i];
		}

		s[num++] = 0;
	}
	else
	{
		//solve the resolvent cubic
		coeffs[0] = 1.0 / 2 * r * p - 1.0 / 8 * q * q;
		coeffs[1] = -r;
		coeffs[2] = -1.0 / 2 * p;
		coeffs[3] = 1;

		SolveCubic(coeffs, temp1);
		for (int i = 0; i < 3; i++)
		{
			s[i] = temp1[i];
		}

		// and take the one real solution
		z = s[0];

		// to build two quadric equations
		u = z * z - r;
		v = 2 * z - p;

		if (IsZero(u))
		{
			u = 0;
		}
		else if (u > 0)
		{
			u = std::sqrt(u);
		}
		else
		{
			return 0;
		}

		if (IsZero(v))
		{
			v = 0;
		}
		else if (v > 0)
		{
			v = std::sqrt(v);
		}
		else
		{
			return 0;
		}

		//first
		coeffs[0] = z - u;
		coeffs[1] = q < 0 ? -v : v;
		coeffs[2] = 1;

		for (auto i = 0; i < 3; i++)
		{
			temp2[i] = coeffs[i];
		}

		num = SolveQuadric(temp2, temp3);

		for (auto i = 0; i < 2; i++)
		{
			s[i] = temp3[i];
		}
		

		//second
		coeffs[0] = z + u;
		coeffs[1] = q < 0 ? v : -v;
		coeffs[2] = 1;

		for (auto i = 0; i < 3; i++)
		{
			temp2[i] = coeffs[i];
		}

		int n = num;

		num += SolveQuadric(temp2, temp3);

		for (auto i = 0; i < 2; i++)
		{
			s[i + n] = temp3[i];
		}
	}

	//resubstitute
	sub = 1.0 / 4 * A;

	for (int i = 0; i < num; ++i)
	{
		s[i] -= sub;
	}

	return num;
}